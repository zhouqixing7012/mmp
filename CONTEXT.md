...
- 当前阶段：个人工作台、资产管理、库存管理、资产盘点主要原型已建立，后续继续按截图和 PRD 逐页校准。
- 2026-08-18 已将 `feat/asset-inventory` 的资产盘点原型整合到最新 `main`：覆盖盘点项目列表、创建/编辑、盘点规则、图片上传规则、盘点范围、快照、盘点计划、计划资产清单、标签扫描、图片审核、项目进度和盘点报告下载入口。
- 资产盘点业务状态按需求控制主要工具栏：暂存 → 快照生成 → 生成盘点计划 → 盘点中 → 盘点关闭；NO/MIS 扫描同步、消息推送、真实权限、随机抽样、真实导入导出等仍为前端演示边界。
- 资产盘点需求原文已存入 `docs/PRD-资产盘点.md`；“公司-账套对应关系”“盘点规则”独立维护页仍缺明确字段，当前不补造独立页面。
...
- 资产盘点分支已完成需求完善并合入最新 `main`；重点验收路径：盘点项目列表 → 创建项目 → 生成快照 → 生成盘点计划 → 启动计划 → 查看计划资产清单/标签扫描；列表还可进入图片审核和项目进度。
...
- 资产盘点：4 个子菜单入口已建立；“盘点项目”已覆盖项目列表、创建/编辑、盘点规则、图片上传规则、盘点范围、快照四页签、执行/未执行/未包含资产管理、默认/自定义盘点计划、计划资产清单、标签扫描、图片审核、项目进度和报告下载入口；独立“公司-账套对应关系”“盘点规则”维护页仍等待明确字段。
