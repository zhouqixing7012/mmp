...
- 2026-08-28 合约号码选择弹窗的单一“合约号码”查询条件改为占满 QueryBar 可用查询宽度，避免默认三列栅格压缩；ES 配给与主管审批“申请信息”里的员工申请附件改为普通“附件”详情字段，字段独占一行，多附件逐行展示“附件名称 + 下载”超链，不再使用附件表格或“操作”列，ES 配给阶段自身“附件信息”仍保留附件大小列。
...
- 合约号码链路当前按“员工申请多附件 → ES 配给多附件 → 主管审批只读同步”演示；ES 同意自动切到同一单据的主管审批页，主管审批后该待办消失，整页刷新可重新开始演示。选择号码弹窗单查询条件已改为全宽；申请信息中的员工附件在 ES 配给和主管审批页均为普通“附件”字段，多附件逐行显示名称和下载链接。
