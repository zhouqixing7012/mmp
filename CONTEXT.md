...
- 2026-08-17 合约号码申请基线已按规则归属拆分：申请信息模块只保留统一规则，身份证号码、申请原因、上传附件分别使用字段级标注；合约号码申请模块 6 个页面当前共 14 条基线标注。
...
- 标注粒度按规则实际归属决定：单字段的必填、只读、脱敏、枚举、输入限制、附件规则等优先生成字段/控件级标注；模块级标注只承载跨字段统一规则、准入/统一校验、状态流转、公共交互和系统副作用。
...
- 合约号码申请页作为粒度示例：`contract-apply-core-rules` 只保留申请级统一流程规则；身份证号码、申请原因、上传附件分别使用独立 field-rule id 和 DetailItem target。
...
- 个人工作台：资产申请、审批、配给、领用、借用、更换、退库、合约号码相关主链路已建立；合约号码申请链路当前 6 个页面共 14 条 PRD 研发评审基线标注。
- 资产管理：资产维护、耗材维护、合约号码维护、标签打印、跨公司转移、资产报废、账面报废、资产处置、员工资产信息查询已建立。
- 库存管理：资产接收、入库、出库、移库、转移、库管员工作台已有主要原型；耗材接收及部分新建/详情页继续按后续字段补充。
- 原型标注：已全局挂载到所有 React 路由；编辑器、本地覆盖层、页面作用域、扩展细粒度目标选择、业务浮层遮挡、SelectModal 语义、查看态点击折叠、Markdown 展示、target/display-anchor 分离、面板最小化和业务模块基线注册均已建立，后续按“字段规则字段化、统一规则模块化”原则继续生成初始 annotation data。
