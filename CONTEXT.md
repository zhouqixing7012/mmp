...
- 2026-08-28 合约号码选择弹窗的单一“合约号码”查询条件改为占满 QueryBar 可用查询宽度，避免默认三列栅格压缩；ES 配给与主管审批“申请信息”里的员工申请附件改为单独一行，仅展示附件名称和操作，不展示大小，ES 配给阶段自身附件信息仍保留大小列。
...
- 合约号码链路当前按“员工申请多附件 → ES 配给多附件 → 主管审批只读同步”演示；ES 同意自动切到同一单据的主管审批页，主管审批后该待办消失，整页刷新可重新开始演示。选择号码弹窗单查询条件已改为全宽；申请信息中的员工附件在 ES 配给和主管审批页均为单独一行，仅显示附件名称与操作。
