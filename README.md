...
后台还包括资产管理、库存管理和资产盘点。库存管理已提供资产接收、耗材接收和入库演示链路：耗材接收支持低值耐用品逐件标签/SN维护，以及低耗耗材接收确认后自动匹配耗材仓并生成采购接收入库结果；入库支持新增入库、采购接收、退库入库、借用归还四种编辑页和各自的物资添加/选择字段。资产盘点当前仅展示一个正式“盘点项目”入口，由原方案三页面链路承接，保留库房/公共/员工盘点范围并排除机房；历史原方案和方案二代码保留但不在菜单展示。“公司-账套对应关系”仍等待明确字段，不补造需求。
