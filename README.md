...
这是一个用于产品演示的企业资产管理前端项目，不是生产系统。当前覆盖员工资产申请、审批、配给、领用、借用、更换、退库、合约号码配给/领取、合约号码退库、后台资产管理、库存管理入口、报废和机房资产等场景。
...
│   ├── contractNumber/              # 合约号码配给、主管审批、库管办理和员工确认
│   ├── assetBorrowing/             # 资产借用
...
```text
合约号码ES配给 → 合约号码配给主管审批 → 合约号码库管员待办 → 员工合约号码领取确认
```

ES 配给需完成电话号码选择和配给附件上传后才能提交主管审批；主管审批页只读展示已填写信息，审批通过后继续原库管办理与员工领取确认流程。
...
- 员工资产信息查询。
- 库存管理一级菜单及 7 个子菜单入口。
- 合约号码 ES 配给必填附件、配给主管审批、库管办理和员工领取确认演示流程。
...
- 合约号码配给附件和主管审批复用现有 Ant Design `Upload / Card / Descriptions / Table`、`StatusTag` 与 `contractNumberAllocationService`，没有新增第三方依赖。
