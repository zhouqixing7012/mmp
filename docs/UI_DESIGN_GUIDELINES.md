...
- Medium：560px
- Large：700～720px
- 数据密集型弹窗可按内容申请更大期望宽度，但**所有业务 Modal / Dialog 最大宽度统一为 960px**
- 实际可用宽度同时不得超过视口宽度减 32px
- 当表格、详情或其他内容的自然宽度超过弹窗最大宽度时，**内容区内部出现横向滚动条，不继续放大 Modal**
- 业务页面不得再直接使用 1120 / 1200 / `94vw` 等方式把弹窗铺满屏幕；确需宽表格时通过 Table `scroll.x` 或内容区 `overflow-x: auto` 承载

公共实现以 `--mmp-modal-max-width: 960px` 为统一出口。
