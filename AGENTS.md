...
**弹窗宽度硬规则**：
- 所有业务 Modal / Dialog 最大宽度统一为 **960px**，且不得超过视口宽度减 32px。
- 普通弹窗优先使用 400 / 560 / 700～720px；数据密集型弹窗可以声明更大期望宽度，但最终仍受 960px 上限约束。
- 内容自然宽度超过弹窗上限时，必须在内容区或 Table 内使用横向滚动；禁止继续把 Modal 撑到 1120 / 1200 / `94vw` 等超宽尺寸。
