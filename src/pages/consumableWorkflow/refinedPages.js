...
export function ConsumableMisApprovalPage() {
  ...
  const application = useMemo(() => (
    getConsumableWorkflowState().applications
      .find((item) => item.status === '处理中' && item.currentNode === 'MIS鉴定') || null
  ), [version]);
  const visibleItems = application?.items.filter((item) => item.requiresMis && item.lineStatus !== '已驳回') || [];
  const mergedValues = useMemo(() => Object.fromEntries(visibleItems.map((item) => [
    item.id,
    {
      misOpinion: values[item.id]?.misOpinion ?? normalizeMisOpinion(item.misOpinion),
      misDescription: values[item.id]?.misDescription ?? item.misDescription,
    },
  ])), [visibleItems, values]);

  const decide = (decision) => {
    if (visibleItems.some((item) => !mergedValues[item.id]?.misOpinion)) {
      messageApi.warning('MIS 意见未选择!');
      return;
    }
    if (visibleItems.some((item) => !mergedValues[item.id]?.misDescription?.trim())) {
      messageApi.warning('意见说明未填写!');
      return;
    }
    const requiredOpinion = decision === '同意' ? '鉴定通过' : '鉴定不通过';
    if (visibleItems.some((item) => mergedValues[item.id]?.misOpinion !== requiredOpinion)) {
      messageApi.warning(`请将全部 MIS 意见选择为“${requiredOpinion}”`);
      return;
    }
    ...
    messageApi.success(decision === '同意' ? 'MIS 鉴定已通过，进入5级审批' : 'MIS 鉴定已驳回相关申请行');
  };
  ...
  const columns = [
    ...
    { title: '配置', dataIndex: 'config', width: 170, render: (value) => value || '-' },
    ...
    { title: '主资产说明', dataIndex: 'mainAssetDesc', width: 220, render: (value) => value || '-' },
    {
      ...
      render: (_, record) => (
        <Select
          className="w-full"
          value={mergedValues[record.id]?.misOpinion || undefined}
          placeholder="请选择"
          options={['鉴定通过', '鉴定不通过'].map((value) => ({ label: value, value }))}
          onChange={(next) => setValues((current) => ({
            ...current,
            [record.id]: { ...mergedValues[record.id], misOpinion: next },
          }))}
        />
      ),
    },
    {
      ...
      render: (_, record) => (
        <TextArea
          ...
          maxLength={400}
          showCount
          value={mergedValues[record.id]?.misDescription || ''}
          onChange={(event) => setValues((current) => ({
            ...current,
            [record.id]: { ...mergedValues[record.id], misDescription: event.target.value },
          }))}
        />
      ),
    },
  ...
  return (
    <Space direction="vertical" size={16} className="w-full">
      ...
      <Card size="small" title="申请耗材信息">
        <Table rowKey="id" size="small" bordered columns={columns} dataSource={visibleItems} pagination={false} scroll={{ x: 1650 }} />
      </Card>
...
export function ConsumableLeaderApprovalPage() {
  ...
  const columns = [
    ...
    { title: '详细说明', dataIndex: 'detail', render: (value) => value || '-' },
  ];
  ...
  return (
    <Space direction="vertical" size={16} className="w-full">
      ...
      <ApplicantCard applicant={application.applicant} applyDate={application.applyDate} />
      <Card size="small" title="申请耗材信息">
        <Table rowKey="id" size="small" bordered columns={columns} dataSource={application.items} pagination={false} scroll={{ x: 1050 }} />
      </Card>
      <ApprovalHistoryCard records={application.history} />
    ...
    <Space direction="vertical" size={16} className="w-full">
      ...
      <Card size="small" title="ES 配给处理">
        <Descriptions bordered size="small" column={3}>
          ...
          <Descriptions.Item label="驳回类型" span={3}>
            <Select
              style={{ width: 320, maxWidth: '100%' }}
              value={rejectType || undefined}
              placeholder="仅驳回时选择"
              allowClear
              options={['取消申请', '取消申请（填写错误）', '取消申请（转为资产申请）'].map((value) => ({ label: value, value }))}
    ...
    <Space direction="vertical" size={16} className="w-full">
      ...
      <Card size="small" title="申请耗材信息">
        <Descriptions bordered size="small" column={3}>
          ...
          <Descriptions.Item label="领用确认方式">
            <Radio.Group value={current.confirmationMode} options={['狐小e电子签', '刷卡确认'].map((value) => ({ label: value, value }))} onChange={(event) => updateField('confirmationMode', event.target.value)} />
          </Descriptions.Item>
          <Descriptions.Item label="确认状态" span={2}><StatusTag value={claim.confirmationStatus} type="business" /></Descriptions.Item>
  ...
  return (
    <Card size="small" title="申请明细">
      <Table
        ...
        bordered
        columns={columns}
        dataSource={rows.filter((row) => row.approved)}
        pagination={false}
        scroll={{ x: 1550 }}
        summary={() => (
          <Table.Summary.Row>
      ...
      <Space direction="vertical" size={16} className="w-full">
        ...
        {groupedApplicants.map((record) => (
          <Card key={record.key} size="small">
            <div className="mb-3 flex flex-wrap items-center gap-x-8 gap-y-2 text-sm text-slate-700">
...
export function ConsumableSummaryApprovalPage() {
  ...
  return (
    <Space direction="vertical" size={16} className="w-full">
      {contextHolder}
      <PageHeader title="耗材汇总审批" numberLabel="汇总单号" number={summary.id} />
      <Card size="small" title="ES汇总说明">
      ...
      <SummaryApplicationTable rows={summary.rows} />
      <Card size="small" title="审批信息">
        <Table
          rowKey={(record, index) => `${record.node}-${index}`}
          size="small"
        ...
        <div className="mt-4">
          ...
          <TextArea className="mt-2" rows={3} maxLength={400} showCount value={comment} placeholder="同意时可不填写，驳回时必填" onChange={(event) => setComment(event.target.value)} />
        </div>
        <div className="mt-4 flex justify-center gap-3">
          <Button type="primary" onClick={() => decide('同意')}>同意</Button>
          <Button danger onClick={() => decide('驳回')}>驳回</Button>
