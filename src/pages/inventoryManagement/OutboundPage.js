...
const OUTBOUND_PURPOSE_OPTIONS = ['员工用机', '部门公用', '其他用途', '专业用途'];

const EMPLOYEES = [
  { value: '206984-何文', label: '206984-何文', employeeNo: '206984', name: '何文', department: 'ERP部.业务产品二组', company: '114.新媒体', costCenter: 'ERP部', city: '010.北京市', building: '129753.搜狐媒体大厦', floor: '15F' },
  { value: '114111-杨芊', label: '114111-杨芊', employeeNo: '114111', name: '杨芊', department: 'ES部.资产管理组', company: '114.新媒体', costCenter: 'ES部', city: '010.北京市', building: '129753.搜狐媒体大厦', floor: '16F' },
  { value: '116201-李娜', label: '116201-李娜', employeeNo: '116201', name: '李娜', department: '技术部.基础设施组', company: '116.天津飞狐', costCenter: '技术部', city: '022.天津市', building: '220101.天津研发中心', floor: '8F' },
];
...
const baseIssueLine = {
  ...
  outboundStatus: '在用-使用中', costCenter: 'ERP部', businessLine: '研发', usage: '员工用机', usageDesc: '日常办公使用',
};
...
const INITIAL_ROWS = [
  ...
  },
  {
    ...
    lines: [{ id: 'mi-1', ...ASSET_POOL[2], department: '技术部.基础设施组', company: '116.天津飞狐', city: '010.北京市', building: '129753.搜狐媒体大厦', floor: '17F', room: 'IDC', outboundStatus: '在用-使用中', costCenter: '技术部', businessLine: '基础设施', usage: '专业用途', usageDesc: '机房部署', quantity: 1, issuePerson: '116201-李娜', issueDate: '2026-09-12', person: '116201-李娜' }],
  },
];
...
function OutboundItemModal({ open, mode, warehouse, initialLine, existingLines, requireSameApprovalChain, onCancel, onConfirm }) {
  ...
  const [form, setForm] = useState({
    ...
    usage: OUTBOUND_PURPOSE_OPTIONS.includes(initialLine?.usage) ? initialLine.usage : '员工用机',
  ...
  const chooseEmployee = (record) => {
    const next = record?.value ? record : EMPLOYEES.find((item) => item.value === record);
    if (!next) return;
    setForm((current) => ({
      ...current,
      person: next.value,
  ...
  const validate = () => {
    ...
    if (requireSameApprovalChain && first && !sameApprovalChain(first, asset)) return '所选资产与当前出库单审批路线不一致，请拆分出库单处理。';
    return '';
  };
  ...
  return (
    <>
      {contextHolder}
      <Modal
        open={open && !selectorOpen && !personSelectorOpen}
          ...
          <Card size="small" title={isBorrow ? '借用出库' : '领用出库'}>
            <DetailGrid columns={3} labelWidth={96} >
              <EditorField label={isBorrow ? '借用人' : '领用人'} required>
                <LookupInput value={form.person} placeholder={isBorrow ? '请选择借用人' : '请选择领用人'} onClick={() => setPersonSelectorOpen(true)} />
              </EditorField>
              ...
              <EditorField label="用途" required><Select className="w-full" value={form.usage} options={OUTBOUND_PURPOSE_OPTIONS.map((v) => ({ label: v, value: v }))} onChange={(v) => set('usage', v)} /></EditorField>
      ...
      <SelectModal
        ...
        columns={[
          ...
          { title: '原值', dataIndex: 'originalValue', width: 120, render: money },
          ...
          { title: '成本中心', dataIndex: 'costCenter', width: 140 },
      ...
      <SelectModal
        ...
        title={isBorrow ? '选择借用人' : '选择领用人'}
        dataSource={EMPLOYEES}
        initialSelectedKeys={form.person ? [form.person] : []}
        rowKey="value"
        columns={[
          { title: '工号', dataIndex: 'employeeNo', width: 120 },
          { title: '姓名', dataIndex: 'name', width: 120 },
          { title: '部门', dataIndex: 'department', width: 240 },
...
function OutboundEditor({ source, onBack, onSave, onStartApproval, onApprove, onReject }) {
  ...
  const cardClaim = source?.autoGenerated ? (source?.cardClaim || '否') : '否';
  const [isSelfPurchase, setIsSelfPurchase] = useState(Boolean(source?.isSelfPurchase));
  ...
  const saveLine = (row, keepOpen) => {
    ...
    if (isSelfPurchase && first && !sameApprovalChain(first, row)) {
  ...
  const validateBeforeSubmit = () => {
    ...
    const first = lines[0];
    if (isSelfPurchase && lines.some((line) => !sameApprovalChain(first, line))) return '所选资产对应的责任审批链不一致，请拆分出库单后重新提交。';
    return '';
  };
  ...
  const importSample = () => {
    const sample = ASSET_POOL.find((asset) => asset.warehouse === warehouse && !asset.locked && AVAILABLE_ASSET_STATUSES.has(asset.assetStatus) && (!isSelfPurchase || !lines[0] || sameApprovalChain(lines[0], asset)));
  ...
  return (
    <Space direction="vertical" size={16} className="w-full" data-page-view-key={`outbound-${status}-${outboundType}`}>
      ...
      {editable && <OutboundItemModal
        ...
        existingLines={lines}
        requireSameApprovalChain={isSelfPurchase}
...
export default function OutboundPage() {
  ...
  const buildRow = (payload, status) => {
    ...
    return {
      ...
      sourceModule: '手工出库',
      isSelfPurchase: Boolean(payload.isSelfPurchase),
      autoGenerated: false,
    };
  };
