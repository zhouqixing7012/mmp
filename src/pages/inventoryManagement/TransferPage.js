...
function SelectorModal({ config, onClose }) {
  ...
  return (
    <SelectModal
      ...
      columns={config.columns || [{ title: '名称', dataIndex: 'name' }]}
      searchFields={config.searchFields || [{ label: '名称', name: 'name', dataIndex: 'name' }]}
      onCancel={onClose}
      onConfirm={(record) => {
        config.onConfirm(record);
        onClose();
      }}
    />
  );
...
function TransferItemModal({ open, currentCompany, initialLine, onCancel, onConfirm }) {
  ...
  const selectorConfig = {
    ...
    receiver: {
      ...
      onConfirm: (record) => setForm((current) => ({ ...current, inPerson: record.name, inDept: record.department || current.inDept })),
    },
  ...
  const submit = (keepOpen) => {
    ...
    const required = [
      ['转入人', form.inPerson],
      ['转入成本中心', form.inCostCenter],
      ['City', form.city],
      ['Building', form.building],
      ['Floor', form.floor],
      ['用途', form.purpose],
      ['资产状态', form.assetStatus],
    ];
    const missing = required.find(([, value]) => !value);
    if (missing) {
      messageApi.warning(`请填写${missing[0]}`);
      return;
    }
    onConfirm({
      ...asset,
      ...
      targetAssetStatus: form.assetStatus,
      ...
      <Space direction="vertical" size={16} className="w-full">
        ...
        <Card size="small" title="选择物资">
          <DetailGrid columns={2} labelWidth={96}>
            <DetailItem label="资产标签号">
              <LookupInput value={asset?.assetTag || ''} placeholder="请选择资产标签号" onOpen={() => setSelectorType('asset')} />
            </DetailItem>
            <DetailItem label="SN号">
              <LookupInput value={asset?.sn || ''} placeholder="请选择SN号" onOpen={() => setSelectorType('asset')} />
            </DetailItem>
          </DetailGrid>
        </Card>
        ...
        <Card size="small" title="转移单信息">
          <DetailGrid columns={4} labelWidth={108} minWidth={1040}>
            <DetailItem label={<RequiredLabel>转入人</RequiredLabel>}>
              <LookupInput value={form.inPerson} placeholder="请选择转入人" onOpen={() => setSelectorType('receiver')} />
            </DetailItem>
            ...
            <DetailItem label={<RequiredLabel>转入成本中心</RequiredLabel>}>
              <Input value={form.inCostCenter} onChange={(event) => update('inCostCenter', event.target.value)} />
            </DetailItem>
            <DetailItem label={<RequiredLabel>City</RequiredLabel>}>
              <LookupInput value={form.city} placeholder="请选择 City" onOpen={() => setSelectorType('city')} />
            </DetailItem>
            <DetailItem label={<RequiredLabel>Building</RequiredLabel>}>
              <LookupInput value={form.building} placeholder="请选择 Building" onOpen={() => setSelectorType('building')} />
            </DetailItem>
            <DetailItem label={<RequiredLabel>Floor</RequiredLabel>}>
              <Select
                className="w-full"
                value={form.floor || undefined}
                placeholder="请选择"
                options={['17层'].map((value) => ({ label: value, value }))}
                onChange={(value) => update('floor', value)}
              />
            </DetailItem>
            <DetailItem label="Room">
              <Input value={form.room} onChange={(event) => update('room', event.target.value)} />
            </DetailItem>
            <DetailItem label={<RequiredLabel>用途</RequiredLabel>}>
              <Select
                className="w-full"
            ...
            <DetailItem label={<RequiredLabel>资产状态</RequiredLabel>}>
              <Select
                className="w-full"
                value={form.assetStatus || undefined}
                placeholder="请选择"
                options={ASSET_STATUS_OPTIONS.map((value) => ({ label: value, value }))}
                onChange={(value) => update('assetStatus', value)}
              />
            </DetailItem>
            <DetailItem label="业务线">
              <LookupInput value={form.businessLine} placeholder="请选择业务线" onOpen={() => setSelectorType('businessLine')} />
            </DetailItem>
            <DetailItem label="项目">
              <LookupInput value={form.project} placeholder="请选择项目" onOpen={() => setSelectorType('project')} />
            </DetailItem>
            <DetailItem label="转移原因" span={3}>
              <Input value={form.transferReason} onChange={(event) => update('transferReason', event.target.value)} />
            </DetailItem>
            <DetailItem label="转移日期">
              <DatePicker
                className="w-full"
                value={form.transferDate ? dayjs(form.transferDate) : null}
                format="YYYY-MM-DD"
                onChange={(date) => update('transferDate', date ? date.format('YYYY-MM-DD') : '')}
              />
            </DetailItem>
            <DetailItem label="使用说明" span={4}>
              <TextArea autoSize={{ minRows: 3, maxRows: 6 }} value={form.usageDescription} onChange={(event) => update('usageDescription', event.target.value)} />
            </DetailItem>
          </DetailGrid>
        </Card>
      </Space>
...
function TransferEditor({ onBack, onSave }) {
  ...
  const columns = [
    ...
    { title: '物资总类', dataIndex: 'materialGroup', width: 120, render: (value) => value || '-' },
    {
      ...
      key: 'assetClass',
      width: 180,
      render: (_, row) => [row.assetClass, row.assetSubClass].filter(Boolean).join(' / ') || '-',
    },
    ...
    <Space direction="vertical" size={16} className="w-full">
      ...
      <Card size="small" title="转移单信息">
        <DetailGrid columns={3} labelWidth={96}>
          ...
          <DetailItem label="公司">
            <LookupInput value={company} placeholder="请选择公司" onOpen={() => setCompanyModalOpen(true)} />
          </DetailItem>
      ...
      <SelectModal
        ...
        title="选择公司"
        dataSource={COMPANY_OPTIONS.map((name, index) => ({ id: index + 1, name }))}
        columns={[{ title: '公司', dataIndex: 'name' }]}
        searchFields={[{ label: '公司', name: 'name', dataIndex: 'name' }]}
        onCancel={() => setCompanyModalOpen(false)}
        onConfirm={(record) => { setCompany(record.name); setCompanyModalOpen(false); }}
      />

      <TransferItemModal
        key={`${lineModalOpen}-${editingLine?.id || 'new'}-${company}`}
        open={lineModalOpen}
...
export default function TransferPage() {
  ...
  const deleteRows = () => {
    if (!selectedRowKeys.length) {
      messageApi.warning('请先选择需要删除的转移单');
      return;
    }
    Modal.confirm({
      title: '确认删除所选转移单？',
  ...
  if (view === 'create') {
    return (
      <>
        {contextHolder}
        <TransferEditor onBack={() => setView('list')} onSave={saveDraft} />
      </>
    );
  }
    ...
    <Space direction="vertical" size={16} className="w-full">
      ...
      <Card size="small" title="转移单列表" extra={<Typography.Text type="secondary">共 {filteredRows.length} 条</Typography.Text>}>
        <div className="mb-3 flex justify-end">
          <Space>
            <Button type="primary" icon={<Plus size={14} />} onClick={() => setView('create')}>创建转移单</Button>
        ...
        <Table
          ...
          rowSelection={{ selectedRowKeys, onChange: setSelectedRowKeys, fixed: true, columnTitle: '选择', columnWidth: 64 }}
          scroll={{ x: 'max-content' }}
