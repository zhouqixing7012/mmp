...
export default function CrossCompanyTransferEditPage({ onBack, variant = 'menu' }) {
  const fromScrapList = variant === 'scrap';
  ...
  const pickerConfig = useMemo(() => {
    ...
    if (selectContext.type === 'formCompany' && fromScrapList) {
      return {
        title: '选择财务公司',
        dataSource: FINANCIAL_COMPANIES,
        searchFields: [
  ...
  }, [buildingData, cityData, fromScrapList, selectContext, warehouseData]);

  const handlePickerConfirm = (record) => {
    if (!selectContext) return;

    if (selectContext.type === 'formCompany') {
      setCompany(fromScrapList ? record.name : formatCodeDesc(record));
  ...
  const handleSubmit = () => {
    ...
    message.success('跨公司转移申请校验通过');
  };

  const renderMenuBasicInfo = () => (
    <Descriptions bordered size="small" column={3} labelStyle={{ width: 128 }}>
      ...
      <Descriptions.Item label={requiredTitle('公司')}>
        <div className="max-w-[360px]">
          {renderPickerInput(company, '请选择公司', { type: 'formCompany' })}
        </div>
      </Descriptions.Item>
      <Descriptions.Item label="单据状态">草稿</Descriptions.Item>
  ...
  const renderScrapListBasicInfo = () => (
    <Descriptions bordered size="small" column={3} labelStyle={{ width: 128 }}>
      ...
      <Descriptions.Item label={requiredTitle('公司')}>
        <div className="max-w-[360px]">
          {renderPickerInput(company, '请选择财务公司', { type: 'formCompany' })}
        </div>
      </Descriptions.Item>
  ...
  return (
    <div className="space-y-4 pb-4 text-gray-800">
      <div className="flex items-start justify-between gap-4 px-1">
        <Title level={3} className="!mb-0 !text-[22px]">跨公司转移申请单</Title>
        <div className="pt-1 text-right">
          <Text type="secondary" className="mb-1 block text-xs">
            {fromScrapList ? '单据编号' : '申请单号'}
          </Text>
          <Text type="secondary">保存/提交后生成</Text>
        </div>
      </div>
      ...
      <Card size="small" title="基本信息" className="shadow-sm">
        {fromScrapList ? renderScrapListBasicInfo() : renderMenuBasicInfo()}
      </Card>
      ...
      <div className="sticky bottom-0 z-40 flex justify-center gap-3 border-t border-[#e5e7eb] bg-white/95 px-5 py-3 shadow-[0_-6px_20px_rgba(15,23,42,0.06)] backdrop-blur">
        <Button className="min-w-[88px]" onClick={onBack}>{fromScrapList ? '退出' : '返回'}</Button>
