...
import { 
  Paperclip, Check, X as CloseIcon, Info, Edit3, Building, FileText, Server, Briefcase, Download, History
} from 'lucide-react';
...
export default function ScrapApproval() {
  ...
  return (
    // 移除了底部固定的超大 padding (原为 pb-28，现改为正常的底部边距 pb-8)
    <div className="min-h-screen bg-[#f4f6f8] flex flex-col font-sans text-sm text-gray-800 pb-8">
      {/* 顶部导航 */}
      ...
      <div className="flex-1 p-4 space-y-6 max-w-[95%] xl:max-w-[1800px] mx-auto w-full mt-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200/80 mt-6 overflow-hidden transition-all hover:shadow-md">
          <div className="bg-gradient-to-r from-gray-50 to-white px-6 py-4 border-b border-gray-100 flex items-center space-x-2">
            ...
            <h3 className="font-bold text-gray-800 text-base">审批历史</h3>
          </div>
          <div className="p-6">
            <div className="overflow-x-auto w-full border border-gray-200/80 rounded-xl">
              <table className="w-full text-left whitespace-nowrap">
                <thead className="bg-gray-50/80 border-b border-gray-200/80 text-gray-600 font-semibold text-[13px]">
                  <tr>
                    <th className="p-3 text-center border-r border-gray-200/80 w-16">序号</th>
                ...
                <tbody className="divide-y divide-gray-100 text-[13px]">
                  {approvalHistoryData.map((row) => (
                    <tr key={row.id} className="hover:bg-gray-50 transition-colors text-gray-700">
                      <td className="p-3 text-center border-r border-gray-100">{row.id}</td>
        ...
        {/* 底部审批操作栏 (作为页面正常流式块，不在悬浮) */}
        <div className="mt-8 bg-white border border-gray-200 shadow-sm rounded-2xl overflow-hidden">
          <div className="px-6 py-5 flex items-center justify-center bg-gray-50/50 gap-6">
            <button className="flex items-center px-10 py-3 text-red-600 bg-white border-2 border-red-100 rounded-xl hover:bg-red-50 hover:border-red-200 transition-all font-bold tracking-wider">
              <CloseIcon className="w-5 h-5 mr-2" /> 驳回
            </button>
            <button className="flex items-center px-14 py-3 text-white bg-blue-600 border-2 border-blue-600 rounded-xl hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-500/30 transition-all font-bold tracking-wider">
