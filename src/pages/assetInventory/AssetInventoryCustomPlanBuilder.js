...
function PlanInfoModal({ open, onCancel, onConfirm }) {
  ...
  const planRangeOptions = PLAN_RANGE_OPTIONS.filter((range) => allowedRanges.includes(range));
  const [planName, setPlanName] = useState('自定义盘点计划');
  const [rows, setRows] = useState(() => planRangeOptions.map((range) => ({ key: range, range, supervisor: '', executor: '' })));
...
export default function AssetInventoryCustomPlanBuilder({ onBack, onConfirmPlan, initialAssets = ASSET_ROWS }) {
  ...
  const generateScope = () => {
    const selectedRanges = filters.methods.filter((range) => allowedRanges.includes(range));
    const row = {
      ...
      floor: filters.floor || '-',
      methods: selectedRanges.length ? selectedRanges : [rangeOptions[0]].filter(Boolean),
  ...
  return <Space direction="vertical" size={16} className="w-full">
    ...
    <Card size="small" title={<CardTitle>盘点范围筛选</CardTitle>}>
      <div className="grid grid-cols-3 gap-x-6 gap-y-3">
        ...
        <div className="col-span-3 flex items-center gap-2"><span className="w-24 shrink-0 text-right text-sm text-gray-600">盘点范围:</span><Checkbox.Group options={rangeOptions} value={filters.methods} onChange={(value) => setFilter('methods', value)} /></div>
      </div>
