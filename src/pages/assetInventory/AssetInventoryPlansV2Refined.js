...
export default function AssetInventoryPlansV2Refined({ project, onBack, onOpenPlanAssets }) {
  ...
  const columns = [
    ...
    ...(project?.projectType === '复盘' ? [{ title: '财务监督人', dataIndex: 'financialSupervisor', width: 130, render: (value, row) => <PersonnelInput disabled={!editable(row)} value={value} onClick={() => openPersonnel(row.key, 'financialSupervisor')} /> }, { title: '内审监督人', dataIndex: 'auditSupervisor', width: 130, render: (value, row) => <PersonnelInput disabled={!editable(row)} value={value} onClick={() => openPersonnel(row.key, 'auditSupervisor')} /> }] : []),
    { title: '盘点范围', dataIndex: 'range', width: 100 }, { title: '资产总量', dataIndex: 'assetCount', width: 100, align: 'right' }, { title: '未盘数量', dataIndex: 'uncountedCount', width: 100, align: 'right' }, { title: '已盘数量', dataIndex: 'countedCount', width: 100, align: 'right' },
