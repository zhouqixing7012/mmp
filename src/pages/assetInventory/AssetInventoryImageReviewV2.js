...
function PhotoPlaceholder({ label }) {
  return (
    <div className="h-24 w-32 rounded border border-dashed border-[#d9d9d9] flex flex-col items-center justify-center text-gray-500 cursor-pointer hover:border-[#1677ff] hover:text-[#1677ff]">
      <ImageIcon size={20} />
...
export default function AssetInventoryImageReviewV2({ onBack }) {
  ...
  const setDecision = (key, decision) => {
    setRows((current) => current.map((row) => (
      row.key === key && row.reviewStatus === '待审核'
        ? { ...row, decision }
        : row
    )));
  };
  ...
  const submitReview = () => {
    ...
    if (pendingSelected.find((row) => !row.decision)) {
      messageApi.warning('所选待审核资产中存在未选择审核结果的分录');
      return;
    }
    setRows((current) => current.map((row) => {
  ...
  return (
    <Space direction="vertical" size={16} className="w-full">
      ...
      <Card size="small" title={<CardTitle>图片审核信息</CardTitle>} extra={<Typography.Text type="secondary">共 {filteredRows.length} 条</Typography.Text>}>
        <Table
          ...
          rowSelection={{
            ...
            getCheckboxProps: (record) => ({ disabled: record.reviewStatus !== '待审核' }),
          }}
          scroll={{ x: 1300 }}
      ...
      <div className="flex justify-center gap-3 pb-2">
        ...
        <Button icon={<Download size={14} />} onClick={() => messageApi.success('图片审核导出已触发')}>图片审核导出</Button>
