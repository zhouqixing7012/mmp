...
export default function BorrowingApplicantCard({ applicant, applyDate, warehouse, onViewAssets }) {
  return (
    <Card title="申请人信息" size="small">
      <Descriptions bordered size="small" column={3}>
        <Descriptions.Item label="申请人">
          <Space size={8}>
            <span>{applicant.name}（{applicant.id}）</span>
            {onViewAssets && (
              <Button
                type="link"
                size="small"
        ...
        <Descriptions.Item label="员工类型">
          <Tag color={applicant.employeeType === '正式员工' ? 'success' : 'default'}>
