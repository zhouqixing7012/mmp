...
import { Button, Card, Descriptions, Empty, Input, Space, Table, Tag, Typography, message as antdMessage } from 'antd';
...
export default function AssetReturnConfirmPage() {
  ...
  const columns = [
    ...
    { title: '配置', width: 260, render: () => application.asset.config || '-' },
    { title: '数量', width: 80, align: 'center', render: () => application.asset.quantity },
    { title: '关联耗材', width: 230, render: () => application.relatedConsumables.length ? application.relatedConsumables.map((item) => <Tag key={item.assetTag} color="blue">{item.assetTag}</Tag>) : '-' },
  ];
      ...
      <Space direction="vertical" size={16} className="w-full">
        ...
        <Card size="small" title="确认信息">
          <Descriptions bordered size="small" column={3}>
            ...
            <Descriptions.Item label="确认状态"><StatusTag value={application.handling.confirmationStatus} type="business" /></Descriptions.Item>
            <Descriptions.Item label="确认时间" span={3}>{application.handling.confirmationTime || '-'}</Descriptions.Item>
          </Descriptions>
        </Card>

        <Card size="small" title="退库资产明细">
          <Table rowKey="id" columns={columns} dataSource={[application.asset]} pagination={false} size="small" bordered scroll={{ x: 1000 }} />
        </Card>

        <Card size="small" title="退库确认提示">
          <Typography.Paragraph>我确认已将上述资产、关联升级耗材及相关附件交还库管员。</Typography.Paragraph>
          <div className="max-w-md">
            <Typography.Text strong><span className="text-red-500">*</span> 确认工号</Typography.Text>
            <Input className="mt-2" value={employeeId} disabled={confirmed} placeholder="请输入申请人工号" onChange={(event) => setEmployeeId(event.target.value)} />
          </div>
        ...
        <div className="flex justify-center gap-3 rounded-lg bg-white p-4 shadow-sm">
          <Button icon={<ScanLine size={14} />} disabled={confirmed} onClick={() => confirm('狐小e扫码')}>模拟扫码确认</Button>
          <Button type="primary" disabled={confirmed} onClick={() => confirm('刷卡')}>模拟刷卡确认</Button>
