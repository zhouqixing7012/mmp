...
export default function ContractReturnConfirmPage() {
  ...
  const confirm = (method, confirmedEmployeeId) => {
    ...
    if (!targetEmployeeId) return messageApi.warning('请输入员工工号');
    try {
  ...
  if (!application) {
    return <>{contextHolder}<Card size="small"><Empty description="暂无待确认的合约号码退库单" /><div className="mt-4 flex justify-center"><Button onClick={() => navigate('/yewurules', { state: { workspace: '工作台首页' } })}>返回工作台</Button></div></Card></>;
  }
  ...
  const detailColumns = [
    ...
    { title: '退库原因', dataIndex: 'returnReason', width: 260, render: (value) => value || '-' },
  ];
  const detailData = [{ ...number, id: number.id || application.id, returnReason: application.reason || '-' }];
    ...
    <>
      ...
      <Space direction="vertical" size={16} className="w-full">
        ...
        <Card size="small" title={<SectionTitle>退库资产明细</SectionTitle>}>
          <Table rowKey="id" size="small" bordered columns={detailColumns} dataSource={detailData} pagination={false} />
        </Card>
        <Card size="small" title={<SectionTitle>退库确认提示</SectionTitle>}>
          <Typography.Paragraph type="danger" strong className="mb-0">提示：我已确认将上述合约号码对应的实体电话卡交还库管员，确认无误后进行刷卡或扫码确认。</Typography.Paragraph>
        </Card>
        <Card size="small" title={<SectionTitle>刷卡/扫码确认</SectionTitle>}>
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_260px] lg:items-center">
            <div>
              ...
              <Space.Compact className="mt-3 w-full max-w-xl">
                <Input value={employeeId} disabled={confirmed} placeholder="请刷员工卡或由管理员录入员工工号" onPressEnter={() => confirm('刷卡确认')} onChange={(event) => setEmployeeId(event.target.value)} />
            ...
            <div className="flex flex-col items-center">
              ...
              <button type="button" disabled={confirmed} className="mt-3 rounded-md border-0 bg-transparent p-0 disabled:cursor-not-allowed disabled:opacity-50" onClick={() => confirm('狐小e扫码确认', application.applicant.id)}>
          ...
          {confirmed && (
            <div className="mt-5">
              <DetailGrid>
                <DetailItem label="识别员工工号">{application.handling.confirmationEmployeeId}（{application.applicant.name}）</DetailItem>
        ...
        <div className="flex justify-center"><Button onClick={() => navigate('/yewurules', { state: { workspace: '合约号码退库办理' } })}>返回</Button></div>
      </Space>
    </>
  );
}
