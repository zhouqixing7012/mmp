...
import { Button, Card, Empty, Input, Modal, Select, Space, Table, Tabs, Tag, Typography, message as antdMessage } from 'antd';
...
export default function AssetReturnApplyPage() {
  ...
  const recordColumns = [
    ...
    { title: '资产说明', width: 230, render: (_, record) => record.asset.assetDesc },
    ...
    { title: '入库单号', width: 170, render: (_, record) => record.handling.inboundOrderNo || '-' },
  ];

  return (
    <div className="min-h-screen bg-slate-100 p-4">
      {contextHolder}
      <Card size="small" title="资产退库">
        <Tabs activeKey={activeTab} onChange={setActiveTab} items={[
          {
            ...
            children: (
              <Space direction="vertical" size={16} className="w-full">
                ...
                <div className="flex justify-center gap-3"><Button type="primary" icon={<Send size={14} />} loading={submitting} onClick={submit}>提交</Button><Button onClick={() => { setSelectedIds([]); setReason(''); }}>返回</Button></div>
              </Space>
            ),
          },
          {
            key: 'records',
            label: '我的退库申请',
            children: <Table rowKey="id" columns={recordColumns} dataSource={applications} pagination={{ pageSize: 10, showTotal: (total) => `共 ${total} 条` }} scroll={{ x: 1400 }} locale={{ emptyText: <Empty description="暂无资产退库申请" /> }} />,
          },
        ]} />
      </Card>
